...
public class Bean2xlsx {
	private Logger logger=LoggerFactory.getLogger(Bean2xlsx.class);
		...
		for (int i = 0; i < data.size(); i++) {
			...
			for (int y = 0; y < colcount; y++) {
				...
				try {
					...
					logger.error("Error getting value from the provided bean!");
					e.printStackTrace();
				}
	...
	private void writeCell(Row row, int col, Object value,
		...
		if (boldFont != null) {
			// font later
			logger.info("font given, but not yet implemented");
		}
		switch (formatType) {
		case TEXT:
		...
		if (bgColor != null) {
			// color later... :)
			logger.info("backgroundcolor given, but not yet implemented");
		}
	}
